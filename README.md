...
- [AnythingMCP](https://github.com/HelpCode-ai/anythingmcp) - Self-hosted gateway that turns systems which are not MCP servers into MCP tools: REST, SOAP/WSDL, GraphQL, SQL/NoSQL databases, and other MCP servers, through a visual editor with no code. Ships 188 pre-built adapters for business software (DATEV, weclapp, Shopware, DHL, Personio, Deutsche Bahn), per-tool response mapping that drops PII before it reaches the model, OAuth2/RBAC/SSO/SCIM, an audit log, and a per-workspace knowledge graph served over MCP so agents chain tools across systems correctly.
