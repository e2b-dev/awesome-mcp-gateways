...
- [Haldir](https://github.com/ExposureGuard/haldir) - MCP proxy/gateway that intercepts every tool call between agents and upstream MCP servers. Enforces scoped sessions with spend limits, policy rules (allow/deny lists, rate limits, time windows, spend caps), human-in-the-loop approvals, and logs everything to an immutable audit trail. Also provides encrypted secrets vault and payment authorization. 10 MCP tools, REST API. 98/100 on Smithery. [haldir.xyz](https://haldir.xyz)
