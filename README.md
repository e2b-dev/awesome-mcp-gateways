...
- [ToolRouter](https://toolrouter.com) - The OpenRouter for tools: one MCP connection (`https://api.toolrouter.com/mcp`) gives an agent 250+ hosted specialist tools and 1,300+ skills across web search, scraping, image and video generation, SEO, finance, property and compliance. A compact `discover` + `use_tool` routing surface keeps the whole catalog at roughly 9k tokens of context. Free tools work immediately; paid tools are pay-per-call from $0.005.
