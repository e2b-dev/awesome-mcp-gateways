...
- [SourceryKit](https://github.com/ProvablyAI/sourcerykit) - Source-available Python SDK and hosted gateway that verifies an agent's outbound requests and MCP tool calls against a source of truth using zero-knowledge proofs, so a call only goes out if its claims check out; it also allow-lists trusted endpoints and logs every call. BSL 1.1.
